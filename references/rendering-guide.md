...
> 文案层规则见 [copy-rules.md](copy-rules.md) + 自检 [copy-checklist.md](copy-checklist.md)。本文件只讲 **JSON 2.0 DSL 的渲染配置**：token / 配色 / 组件结构 / 布局，这些只能从真实 .card 提取。
>
> 样本在 [../samples/](../samples/)（见 [index](../samples/index.md)），5 张全是手工做的正面 case。
...
## 6b. 纵向堆叠 vs 左右并排（软规则，别写死）

核心亮点 / 适用场景 / 功能块，排成**纵向堆叠**（单列、每块占满宽）还是**左右并排**（`column_set` 2 列、每块半宽），**不写死、不严格按内容**：

- **默认：随机选，或直接问用户**（"想要纵向一列还是左右两列？"）——让卡片之间有变化，别千篇一律。
- **唯一红线：长句 / 长描述不走左右分裂**。窄列里长文本疯狂换行、两列高度还对不齐。**长内容 → 必纵向**。
- 内容长短**只是参考、不是铁律**：单项短 → 左右并排更紧凑也行；单项偏长 → 倾向纵向。
- 配合 [copy-checklist.md](copy-checklist.md) #2：无论哪种布局，同组各块描述长度先拉齐。

判定优先级：**有长内容？→ 必纵向**。否则 → 随机 / 问用户 / 按长短倾向，任选其一。
